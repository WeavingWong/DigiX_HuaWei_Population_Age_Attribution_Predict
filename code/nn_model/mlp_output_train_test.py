...
gc.collect()
tf_vec = TfidfVectorizer(lowercase=False,ngram_range=(1,2),min_df=0.0001,token_pattern='(?u)\\b\\w+\\b')
tf_vec.fit(x_train + x_test)
x_train = tf_vec.transform(x_train).toarray()
x_test = tf_vec.transform(x_test).toarray()
...
y_val.to_csv('../../data/prob_file/mlp_output_train.csv')
y_test = pd.DataFrame(y_test,index=x_test.uId.tolist())
y_test.to_csv('../../data/prob_file/mlp_output_test.csv')
