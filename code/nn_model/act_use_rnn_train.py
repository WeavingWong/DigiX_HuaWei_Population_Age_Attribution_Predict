...
usage_list = pd.read_csv('../input/usage_app_info.csv')   #重采样的usage_app
usage_appId = pd.read_csv('../input/usage_appId.csv')     #激活表的app词典
appId = pd.read_csv('../input/appId.csv')   #使用表的app词典
...
for index,row in tqdm(user_app_actived.iterrows()):
    try:
        result.append(np.sort(set(row['app_list_x']) | set(row['app_list_y'])))
    except:
...
del usage_list
gc.collect()
x_train = pd.read_csv('../input/age_train.csv',names=['uId','age_group'],dtype={'uId':np.int32, 'age_group':np.int8})
x_test = pd.read_csv('../input/age_test.csv',names=['uId'],dtype={'uId':np.int32})
...
gc.collect()
usage_appId = pd.read_csv('../input/usage_appId_top_num100000.csv')
...
app_list = sequence.pad_sequences(app_list, maxlen=150)
app_test = sequence.pad_sequences(app_test, maxlen=150)
...
for i, (train_index, valid_index) in enumerate(kfold.split(app_list, np.argmax(y_train,axis=1))):
    ...
    model = RnnVersion2(max_len = 150,emb_size = 12697)
...
y_valb.to_csv('../input/act_use_rnn_train.csv')
y_testb = pd.DataFrame(y_testb,index=x_test.uId)
y_testb.to_csv('../input/act_use_rnn_test.csv') 
