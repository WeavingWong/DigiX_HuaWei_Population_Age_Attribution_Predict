...
app_list = [[x for x in apps if  x in app_dict] for apps in x_train.app_list]
app_test = [[x for x in apps if  x in app_dict] for apps in x_test.app_list]
app_list = [" ".join(app) for app in app_list]
app_test = [" ".join(app) for app in app_test]

c_vec1 = CountVectorizer(lowercase=False,ngram_range=(1,1),dtype=np.int8)
c_vec1.fit(app_list)
app_list = c_vec1.transform(app_list).toarray()
app_test = c_vec1.transform(app_test).toarray()
...
y_valb.to_csv('../../data/features/act_use_train_mlp.csv')
y_testb = pd.DataFrame(y_testb,index=x_test.uId)
y_testb.to_csv('../../data/features/act_use_test_mlp.csv') 
