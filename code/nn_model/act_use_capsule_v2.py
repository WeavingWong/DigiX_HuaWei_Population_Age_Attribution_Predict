...
"""
功能：拼接使用表 和 激活表的appid,使用表取使用次数top20000的app,生成一个用户对应的特征app列表,
最后用rnn模型进行交叉验证训练，得到输出层和中间层结果。
...
usage_appId = pd.read_csv('../../data/processed_data/usage_appId.csv')     #激活表的app词典
appId = pd.read_csv('../../data/processed_data/appId.csv')   #使用表的app词典
...
y_test_prob = np.zeros((x_test.shape[0],160))
y_val_prob = np.zeros((x_train.shape[0],160))
...
for i, (train_index, valid_index) in enumerate(kfold.split(app_list, np.argmax(y_train,axis=1))):
    ...
    y_val_prob[valid_index] = dense1_layer_model.predict(X_val, batch_size=128, verbose=1)

    y_test_prob += np.array(dense1_layer_model.predict(app_test, batch_size=128, verbose=1))/5

y_valb = pd.DataFrame(y_valb,index=x_train.uId)
y_valb.to_csv('../data/feature/act_use_capsule_train_v3.csv',index=False)
y_testb = pd.DataFrame(y_testb,index=x_test.uId)
y_testb.to_csv('../data/feature/act_use_capsule_test_v3.csv', index=False) 

y_val_prob = pd.DataFrame(y_val_prob,index=x_train.uId)
y_val_prob.to_csv('../data/feature/act_use_rnn_hide_train_v3.csv',index=False)
y_test_prob = pd.DataFrame(y_test_prob,index=x_test.uId)
y_test_prob.to_csv('../data/feature/act_use_rnn_hide_test_v3.csv',index=False)
