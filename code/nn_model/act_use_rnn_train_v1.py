...
import numpy as np
import pandas as pd
from models import RnnVersion1 
import gc
...
y_test_prob = np.zeros((x_test.shape[0],270))
y_val_prob = np.zeros((x_train.shape[0],270))
...
y_valb.to_csv('../../data/prob_file/act_use_rnn_train_v1.csv',index=False)
y_testb = pd.DataFrame(y_testb,index=x_test.uId)
y_testb.to_csv('../../data/prob_file/act_use_rnn_test_v1.csv', index=False) 

y_val_prob = pd.DataFrame(y_val_prob,index=x_train.uId)
y_val_prob.to_csv('../../data/features/act_use_rnn_hide_train.csv',index=False)
y_test_prob = pd.DataFrame(y_test_prob,index=x_test.uId)
y_test_prob.to_csv('../../data/features/act_use_rnn_hide_test.csv',index=False)