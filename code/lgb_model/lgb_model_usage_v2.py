...
actived_rnn_features_train_v1 = sparse.load_npz('../../data/csr_features_full/actived_rnn_features_train_usage_v1.npz')
actived_rnn_features_test_v1 = sparse.load_npz('../../data/csr_features_full/actived_rnn_features_test_usage_v1.npz')
...
for index, (train_index, valid_index) in enumerate(skf.split(train, label)):
    ...
    gc.collect()
    joblib.dump(model, "../../data/output/lgb_model_usage_csr_v2_{}.m".format(index))      # 保存模型
...
y_val.to_csv('../../data/output/lgb_valid_usage_csr_v2.csv', index=False, encoding='utf-8')
# 保存测试集概率
y_test = pd.DataFrame(y_test)
y_test = pd.concat([test_uid,y_test],axis=1)
y_test.to_csv('../../data/output/lgb_test_usage_csr_v2.csv', index=False, encoding='utf-8')
# 保存每个模型验证集对应的分数
best_score = pd.DataFrame(best_score)
best_score.to_csv('../../data/output/lgb_model_score_usage_csr_v2.csv', index=False, encoding='utf-8')
print("OK")
end_time = time.time()
