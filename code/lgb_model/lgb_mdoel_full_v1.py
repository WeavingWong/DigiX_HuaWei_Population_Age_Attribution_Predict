...
for index, (train_index, valid_index) in enumerate(skf.split(train, label)):
    ...
    gc.collect()
    joblib.dump(model, "../../data/output/lgb_model_full_csr_v1_{}.m".format(index))      # 保存模型
...
y_val.to_csv('../../data/output/lgb_valid_full_csr_v1.csv', index=False, encoding='utf-8')
# 保存测试集概率
y_test = pd.DataFrame(y_test)
y_test = pd.concat([test_uid,y_test],axis=1)
y_test.to_csv('../../data/output/lgb_test_full_csr_v1.csv', index=False, encoding='utf-8')
# 保存每个模型验证集对应的分数
best_score = pd.DataFrame(best_score)
best_score.to_csv('../../data/output/lgb_model_score_full_csr_v1.csv', index=False, encoding='utf-8')
print("OK")
end_time = time.time()
