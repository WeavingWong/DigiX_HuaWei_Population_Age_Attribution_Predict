...
train_output_prod = pd.merge(age_train[['uId']], mlp_output_train_cnt, how='inner', on='uId').fillna(0)
test_output_prod = pd.merge(age_test, mlp_output_test_cnt, how='inner', on='uId').fillna(0)
...
train = pd.merge(train_output_prod, mlp_output_train, how='inner', on='uId').fillna(0)
test = pd.merge(test_output_prod, mlp_output_test, how='inner', on='uId').fillna(0)
...
train = pd.merge(train_output_prod, act_use_rnn_train, how='inner', on='uId').fillna(0)
test = pd.merge(test_output_prod, act_use_rnn_test, how='inner', on='uId').fillna(0)
...
train_output_prod = pd.merge(train_output_prod, act_use_rnn_train_v1, how='inner', on='uId').fillna(0)
test_output_prod = pd.merge(test_output_prod, act_use_rnn_test_v1, how='inner', on='uId').fillna(0)
...
train_output_prod = pd.merge(train_output_prod, act_use_rnn_train_v2, how='inner', on='uId').fillna(0)
test_output_prod = pd.merge(test_output_prod, act_use_rnn_test_v2, how='inner', on='uId').fillna(0)
...
train_output_prod = pd.merge(train_output_prod, act_use_rnn_train_mlp, how='inner', on='uId').fillna(0)
test_output_prod = pd.merge(test_output_prod, act_use_rnn_test_mlp, how='inner', on='uId').fillna(0)
...
train_output_prod = pd.merge(train_output_prod, act_all_train_mlp, how='inner', on='uId').fillna(0)
test_output_prod = pd.merge(test_output_prod, act_all_test_mlp, how='inner', on='uId').fillna(0)

# LGB模型的结果概率文件1(不加NN，不加usage)12
...
train_output_prod = pd.merge(train_output_prod, lgb_valid_full_csr_v1, how='inner', on='uId').fillna(0)
test_output_prod = pd.merge(test_output_prod, lgb_test_full_csr_v1, how='inner', on='uId').fillna(0)

# LGB模型的结果概率文件2(加NN，不加usage)13
...
train_output_prod = pd.merge(train_output_prod, lgb_valid_full_csr_v2, how='inner', on='uId').fillna(0)
test_output_prod = pd.merge(test_output_prod, lgb_test_full_csr_v2, how='inner', on='uId').fillna(0)

# LGB模型的结果概率文件3(不加NN，加usage)14
lgb_valid_usage_csr_v1=pd.read_csv('../../data/prob_file/lgb_valid_usage_csr_v1.csv')
lgb_valid_usage_csr_v1=memory_preprocess._memory_process(lgb_valid_usage_csr_v1)
lgb_test_usage_csr_v1=pd.read_csv('../../data/prob_file/lgb_test_usage_csr_v1.csv')
lgb_test_usage_csr_v1=memory_preprocess._memory_process(lgb_test_usage_csr_v1)
train_output_prod = pd.merge(train_output_prod, lgb_valid_usage_csr_v1, how='inner', on='uId').fillna(0)
test_output_prod = pd.merge(test_output_prod, lgb_test_usage_csr_v1, how='inner', on='uId').fillna(0)

# LGB模型的结果概率文件4(加NN，加usage)15
lgb_valid_usage_csr_v2=pd.read_csv('../../data/prob_file/lgb_valid_usage_csr_v2.csv')
lgb_valid_usage_csr_v2=memory_preprocess._memory_process(lgb_valid_usage_csr_v2)
lgb_test_usage_csr_v2=pd.read_csv('../../data/prob_file/lgb_test_usage_csr_v2.csv')
lgb_test_usage_csr_v2=memory_preprocess._memory_process(lgb_test_usage_csr_v2)
train_output_prod = pd.merge(train_output_prod, lgb_valid_usage_csr_v2, how='inner', on='uId').fillna(0)
test_output_prod = pd.merge(test_output_prod, lgb_test_usage_csr_v2, how='inner', on='uId').fillna(0)

# NN模型的结果概率文件16
dnn_output_train_usage=pd.read_csv('../../data/prob_file/dnn_output_train_usage.csv')
dnn_output_train_usage=memory_preprocess._memory_process(dnn_output_train_usage)
dnn_output_test_usage=pd.read_csv('../../data/prob_file/dnn_output_test_usage.csv')
...
train_output_prod = pd.merge(train_output_prod, dnn_output_train_usage, how='inner', on='uId').fillna(0)
test_output_prod = pd.merge(test_output_prod, dnn_output_test_usage, how='inner', on='uId').fillna(0)
