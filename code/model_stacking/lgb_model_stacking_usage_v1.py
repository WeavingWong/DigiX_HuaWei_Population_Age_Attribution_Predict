...
actived_rnn_features_train_v1 = sparse.load_npz('../../data/csr_features_usage/actived_rnn_features_train_usage_v1.npz')
actived_rnn_features_test_v1 = sparse.load_npz('../../data/csr_features_usage/actived_rnn_features_test_usage_v1.npz')
...
# 导入usage表rnn_features特征
usage_rnnV2_train_usage_160 = sparse.load_npz('../../data/csr_features/usage_rnnV2_train_usage_160.npz')
usage_rnnV2_test_usage_160 = sparse.load_npz('../../data/csr_features/usage_rnnV2_test_usage_160.npz')
...
train = sparse.hstack((train,usage_rnnV2_train_usage_160))
test = sparse.hstack((test,usage_rnnV2_test_usage_160))
print(train.shape)
print(test.shape)
del usage_rnnV2_train_usage_160,usage_rnnV2_test_usage_160
gc.collect()
...
# joblib.dump(model, "../../data/output/lgb_model_stacking_usage_v1.m")      # 保存模型
...
for index, (train_index, valid_index) in enumerate(skf.split(train, label)):
    ...
    gc.collect()
    joblib.dump(model, "../../data/output/lgb_model_stacking_usage_v1_{}.m".format(index))      # 保存模型
    # model = joblib.load("../../data/output/lgb_model_stacking_usage_v1_{}.m".format(index))     # 加载模型
...
y_val.to_csv('../../data/output/lgb_valid_stacking_usage_v1.csv', index=False, encoding='utf-8')
# 保存测试集概率
y_test = pd.DataFrame(y_test)
y_test = pd.concat([test_uid,y_test],axis=1)
y_test.to_csv('../../data/output/lgb_test_stacking_usage_v1.csv', index=False, encoding='utf-8')
# 保存每个模型验证集对应的分数
best_score = pd.DataFrame(best_score)
best_score.to_csv('../../data/output/lgb_model_score_stacking_usage_v1.csv', index=False, encoding='utf-8')
print("OK")
end_time = time.time()
